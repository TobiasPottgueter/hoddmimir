...
final readonly class PolicySelectionPage
{
    /** @param list<PolicySelectionEntry> $items */
    public function __construct(public PageRequest $page, public array $items, public ?PageCursor $nextCursor)
    {
        if (count($items) > $page->limit || (null !== $nextCursor && [] === $items)) {
            throw new InvalidArgumentException('The policy selection page is invalid.');
        }
    }
    ...
    {
        return [
            'items' => array_map(static fn (PolicySelectionEntry $item): array => $item->toArray(), $this->items),
            'page' => [
