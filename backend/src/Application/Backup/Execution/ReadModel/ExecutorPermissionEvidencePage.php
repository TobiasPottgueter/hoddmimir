...
final readonly class ExecutorPermissionEvidencePage
{
    /** @param list<ExecutorPermissionEvidenceItem> $items */
    public function __construct(
        ...
        if (count($items) > $page->limit || (null !== $nextCursor && [] === $items)) {
            throw new InvalidArgumentException('The executor permission evidence page is invalid.');
        }
    }
    ...
    public function toArray(): array
    {
        return [
            'items' => array_map(
                static fn (ExecutorPermissionEvidenceItem $item): array => $item->toArray(),
                $this->items,
            ),
            'page' => [
