...
final class RunAutomaticShadowEvaluationTest extends TestCase
{
    public function testMissingExecutorEvidenceBlocksAndPersistsAllGatesInStableOrder(): void
    {
        ...
        self::assertSame([
            ...
            'policy_failure_notification_configured', 'target_enabled', 'explicit_exclusion_absent', 'guest_active', 'inventory_fresh',
        ...
        self::assertSame('missing', $decision->gates[22]->result->detailCode->value);
    }
    ...
    public function testEvidenceExactlyThreeHundredSecondsOldIsFreshAndIdsAreStable(): void
    {
        ...
        $service->execute($this->lease());
        self::assertTrue($store->batches[0]->decisions[0]->gates[9]->result->passed);
        self::assertTrue($store->batches[0]->decisions[0]->gates[11]->result->passed);
    ...
    public function testFreshAuthorizedExecutorEvidencePassesBothAuthorizationGates(): void
    {
        ...
        self::assertTrue($decision->gates[22]->result->passed);
    ...
    public function testMissingExpectedBackupSizeEvidenceBlocksWithoutPromotion(): void
    {
        ...
        $decision = $store->batches[0]->decisions[0];
        self::assertSame(DecisionOutcome::Blocked, $decision->outcome);
        self::assertSame('minimum_free_space', $decision->gates[16]->result->code->value);
        self::assertSame('missing', $decision->gates[16]->result->detailCode->value);
    ...
    public function testPerGuestWinnerUsesReasonBeforePolicyPriorityAndExplainsTheLoser(): void
    {
        ...
        self::assertSame('higher_ranked_candidate_absent', $loser->gates[25]->result->code->value);
        self::assertSame('higher_ranked_candidate', $loser->gates[25]->result->detailCode->value);
    }
    ...
    {
        foreach ([
            'stale' => [new DateTimeImmutable('2026-07-12T09:59:59.999999Z'), true, 21, 'stale'],
            'unauthorized' => [new DateTimeImmutable('2026-07-12T10:00:00Z'), false, 22, 'unauthorized'],
    ...
    public function testActiveRequestProducesClosedDeduplicatedExplainability(): void
    {
        ...
        self::assertSame('active_request_absent', $decision->gates[24]->result->code->value);
        self::assertSame('active_request_exists', $decision->gates[24]->result->detailCode->value);
    }
    ...
    public function testIncompatiblePolicyRetentionBlocksBeforePromotion(): void
    {
        ...
        $decision = $store->batches[0]->decisions[0];
        self::assertSame(DecisionOutcome::Blocked, $decision->outcome);
        self::assertSame('policy_retention_compatible', $decision->gates[23]->result->code->value);
        self::assertSame('incompatible', $decision->gates[23]->result->detailCode->value);
    }

    public function testUnconfiguredFailureNotificationIsDistinctFromPolicyStatus(): void
    {
        $store = new CapturingShadowStore();
        $this->service(
            new RecordingAutomaticShadowSource([$this->candidate(policyFailureNotificationConfigured: false)]),
            $store,
            new DateTimeImmutable('2026-07-12T10:05:00Z'),
        )->execute($this->lease());
        ...
        $decision = $store->batches[0]->decisions[0];
        self::assertSame(DecisionOutcome::Blocked, $decision->outcome);
        self::assertTrue($decision->gates[4]->result->passed);
        self::assertSame('policy_enabled', $decision->gates[4]->result->code->value);
        self::assertFalse($decision->gates[5]->result->passed);
        self::assertSame('policy_failure_notification_configured', $decision->gates[5]->result->code->value);
        self::assertSame('unconfigured', $decision->gates[5]->result->detailCode->value);
        self::assertSame([], $store->batches[0]->promotions);
    }
    ...
    {
        foreach ([
            'node' => [false, true, 17, 18],
            'target' => [true, false, 18, 17],
    ...
    public function testCompleteCounterEvidenceSelectsBytesReasonAndMissingPlacementStaysExplicit(): void
    {
        ...
        self::assertFalse($decision->gates[10]->result->passed);
    ...
    public function testCompoundGuestAndCapacityGateBranchesRemainExplicit(): void
    {
        foreach ([
            ...
            $store = new CapturingShadowStore();
            $this->service($source, $store, new DateTimeImmutable('2026-07-12T10:05:00Z'))->execute($this->lease());
            self::assertCount(25, $store->batches[0]->decisions[0]->gates);
        }
    }
    ...
    public function testByteCounterFreshnessAndResetCannotUseMissingStaleOrFutureEvidence(): void
    {
        $now = new DateTimeImmutable('2026-07-12T10:05:00Z');
        foreach ([null, $now->modify('-301 seconds'), $now->modify('+1 second'), $now->modify('-300 seconds'), $now] as $at) {
            foreach ([1000, 0] as $current) {
                $source = new RecordingAutomaticShadowSource([$this->candidate(
                    lastSuccessAt: new DateTimeImmutable('2026-07-12T09:00:00Z'), maximumAgeSeconds: 604800,
                    currentBytes: $current, baselineBytes: 100, bytesThreshold: 100, cooldownSeconds: 60, writeObservedAt: $at,
                )]);
                $store = new CapturingShadowStore();
                $this->service($source, $store, $now)->execute($this->lease());
                $fresh = null !== $at && $at <= $now && $at >= $now->modify('-300 seconds');
                self::assertSame($fresh && 1000 === $current ? 'bytes_written' : null, $store->batches[0]->decisions[0]->reasonPriority?->reason->value);
                self::assertCount($fresh && 0 === $current ? 1 : 0, $source->resets);
            }
        }
        $source = new RecordingAutomaticShadowSource([$this->candidate(
            lastSuccessAt: new DateTimeImmutable('2026-07-12T09:00:00Z'), maximumAgeSeconds: 60,
            currentBytes: 1000, baselineBytes: 100, bytesThreshold: 100, cooldownSeconds: 60, writeObservedAt: null,
        )]);
        $store = new CapturingShadowStore();
        $this->service($source, $store, $now)->execute($this->lease());
        self::assertSame('max_age', $store->batches[0]->decisions[0]->reasonPriority?->reason->value);
    ...
    private function candidate(
        ...
        bool $policyFailureNotificationConfigured = true,
        bool $policyRetentionCompatible = true,
        ?string $policyId = null,
        int $policyPriority = 0,
        bool $includeResolvedPolicyJson = true,
        ?DateTimeImmutable $writeObservedAt = new DateTimeImmutable('2026-07-12T10:00:00Z'),
        ...
        return new AutomaticShadowCandidate(
            ...
            $placementPresent ? $id('node') : null, true, $placementPresent ? 2 : null, $placementPresent ? $at : null, $policyId ?? $id('policy'), 3, true, $policyFailureNotificationConfigured, $policyRetentionCompatible, $policyRetentionCompatible ? hash('sha256', $json, true) : hash('sha256', 'policy', true),
            ...
            null === $currentBytes ? null : new UInt64Decimal((string) $currentBytes), $writeObservedAt,
